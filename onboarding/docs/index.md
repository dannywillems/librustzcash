...
description: A graduate-level reading course on librustzcash, with deep focus on the cryptography.
---
...
The chapters are ordered as a study plan: you can read them in
sequence, but each is also written to be useful on its own as a
reference once you know your way around.

## How to read this course

LaTeX is rendered via KaTeX (configured in this site). The math is
the substance of the chapters, not decoration.
...
The course splits into three parts. Part I is the reading course
(narrative). Part II goes deeper into bugs, side channels, audits,
and operational rigour. Part III is authoritative reference
material (key catalog, circuit walk-through).
...
| # | Title | Focus |
| --- | --- | --- |
| 23 | [The complete key catalog](./23-key-catalog.md) | Every key symbol defined, derived, typed, and located in code |
...
- $\langle \cdot, \cdot \rangle$: a pairing
  $e : \mathbb{G}_1 \times \mathbb{G}_2 \to \mathbb{G}_T$ when used
  in pairing contexts.
- $\mathsf{Com}(m; r)$: a commitment to message $m$ with randomness
  $r$.
- $\mathsf{Enc}_k, \mathsf{Dec}_k$: symmetric encryption/decryption
  with key $k$.
...
- `zcashd` consensus implementation (for cross-checking):
  https://github.com/zcash/zcash
- Zebra (Rust full node): https://github.com/ZcashFoundation/zebra
- `sapling-crypto` (external repo):
